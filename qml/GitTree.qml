...
Flickable {
    ...
    // TODO!SV do better, canvas.height?
    contentWidth: 600
    contentHeight: commits_view.contentHeight
    clip: true
...
//        anchors { fill: true }
        ...
        function getX(index){
            return (index + 0.5) * canvas.branchColWidth;
        }
        ...
        function getBranchIndex(pixvalue){
            return parseInt(pixvalue / canvas.branchColWidth, 10);
        }
        ...
        function shiftToCircleStart(point){
            return [point[0], point[1] + 0.5*canvas.circleWidth];
        }
        ...
        function getBezierCurvePoints(point, nextPoint){
            ...
            var wayPointY = point[1] + root.rowHeight - 0.5*canvas.circleWidth;
        ...
        onPaint: {
            ...
            var istart = Math.max(canvas.getCommitIndex(commits_view.contentY) - 1, 0);
            var iend   = Math.min(Math.max(canvas.getCommitIndex(commits_view.contentY + getCommitPosRange()) + 1,
                                           0),
                                  commits_view.count);
        ...
        MouseArea {
            ...
            onPositionChanged: {
                var icoords = canvas.getCommitAndBranchIndex([mouse.x, mouse.y]);
